...
from ERICA.utils.dbfunc import get_couple, save_couple
...
@app.on_message(filters.command("shifting") & ~filters.edited)
async def couple(_, message):
    if message.chat.type == "private":
        await message.reply_text("This command only works in groups.")
        return
    try:
        ...
        if not is_selected:
            ...
            couple_selection_message = f"""**ɴɪʙʙᴀ ɴɪʙʙɪ ᴏꜰ ᴛʜᴇ ᴅᴀʏ**
{c1_mention} + [𓆩𝐌𝐈𝐒𝐒 𝐑𝐈𝐑𝐔𝐑𝐔𓆪](https://t.me/RIRURU_OFFICIAL) = ❤️
__ɴᴇᴡ ᴄᴏᴜᴘʟᴇ ᴏꜰ ᴛʜᴇ ᴅᴀʏ ᴍᴀʏ ʙᴇ ᴄʜᴏꜱᴇɴ ᴀᴛ 12ᴀᴍ {tomorrow}__"""
            await app.send_message(
                message.chat.id,
                text=couple_selection_message
            ...
            c2_name = (await app.get_users(c2_id)).first_name
            couple_selection_message = f"""Couple of the day:
[{c1_name}](tg://openmessage?user_id={c1_id}) + [𓆩𝐌𝐈𝐒𝐒 𝐑𝐈𝐑𝐔𝐑𝐔𓆪](tg://openmessage?user_id=5010530669) = ❤️
__New couple of the day may be chosen at 12AM {tomorrow}__"""
            await app.send_message(
                message.chat.id,
                text=couple_selection_message
